...
Other uses may tagging different interfaces and running an instance for each. [You can view those tagging options below](https://github.com/breadlysm/speedtest-to-influxdb/blob/master/README.md#tag-options) 
 ...
 The grafana image below is a prebuilt dashboard json named `Grafana_SpeedTests-by-SpeedFlux.json.json`:

![OriginalDash](.images/1.png)

Additionally, there is an evolution called `Grafana_SpeedTests-by-SpeedFlux (Evolution).json`:

![OriginalDash](.images/2.png)
...
- SPEEDTEST_INTERVAL = default - 30 (minutes)
